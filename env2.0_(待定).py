...
class CarChangeLaneEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # —————————————————————————————————————————————lstm轨迹加在这里——-----------——------------------------------------
        self.lstmTrace = pd.read_csv('LSTM_Trace/525_LSTM.csv')
        self.index = 1
        self.overIndex = 1
    ...
    def step(self, action):
        ...
        if np.sqrt(d_pre_goal ** 2 + interval_pre ** 2) < 3 \
                or np.sqrt(d_tail_goal ** 2 + interval_tail ** 2) < 3:
            self.isCrashed = True
        ...
        if done:
            ...
            if 2 / 3 * abs(self.y_pre_raw - self.y_tail_goal) < abs(interval_tail) \
                    <= abs(self.y_pre_raw - self.y_tail_goal):
                S = TTC3 / 2220
                typ = 1
            elif 1 / 3 * abs(self.y_pre_raw - self.y_tail_goal) < abs(interval_tail) \
                    <= 2 / 3 * abs(self.y_pre_raw - self.y_tail_goal):
            ...
            #
            # self.index += 1
            print('type:{}, S:{}, C:{}, E:{}'.format(typ, S, C, E))
            ...
            #     reward = -100
                ...
                reward = 5 * 0.607 * S + 0.184 * C + 2 * 0.209 * E - bias
